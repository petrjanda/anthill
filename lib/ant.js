...
Ant.prototype.on = function(ant, name, cb) {
  ...
  self._connections[ant].client.on(name, cb);
};

Ant.prototype.connect = function(ant, cb) {
  var self = this;
  
  dnode.connect(ant, function (client, connection) {
    connection.on('end', function () {
      self.emit('end', ant);
    });
    
    self.emit('connect', ant);
    
    self._connections[ant] = {
      client: client, 
